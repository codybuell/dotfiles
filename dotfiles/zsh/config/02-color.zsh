...
# Color
#
# Main entrypoint for switching and managing color schemes in the shell.
# Extracts color values from scheme files using Zsh parameter expansion,
# avoiding external dependencies, and applies colors to Kitty and tmux
# configurations. Handles race conditions when updating configs.
#
# Usage:
#   `color help`           — shows usage
#   `color ls [pattern]`   — lists available schemes
#   `color rand [pattern]` — picks a random scheme
#   `color -`              — restores previous scheme
#   `color [scheme]`       — applies a specific scheme
color() {
  emulate -L zsh
  ...
  __color() {
    ...
    if [[ -e "$FILE" ]]; then
      ...
      if [ -e "$__BUELL[TINTED_CONFIG]" ]; then
...
EOF
  ...
  if [ $# -eq 0 ]; then
    if [ -s "$__BUELL[TINTED_CONFIG]" ]; then
      cat "$__BUELL[TINTED_CONFIG]"
      SCHEME=$(head -1 "$__BUELL[TINTED_CONFIG]")
      __color "$SCHEME"
      return
    else
      SCHEME=help
...
# Anonymous Function
#
# Automatically apply the last used else default color scheme.
function () {
  emulate -L zsh
  ...
  if [[ $SHLVL -eq 1 || ! -s ~/.config/tmux/colors.conf ]]; then
    ...
    if [[ -s "$__BUELL[TINTED_CONFIG]" ]]; then
      ...
      if [ "$BACKGROUND" != 'dark' -a "$BACKGROUND" != 'light' ]; then
        echo "warning: unknown background type in $__BUELL[TINTED_CONFIG]"
      fi
      color "$SCHEME"
    else
      # Default
