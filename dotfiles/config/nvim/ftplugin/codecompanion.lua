...
local refresh_syntax = function(force)
  if vim.bo.filetype == 'codecompanion' and vim.fn.line('$') > 1 and vim.bo.syntax ~= 'OFF' then
...
-- autocommands for codecompanion codeblock syntax
augroup('BuellCodeCompanion', function()
  ...
  autocmd('InsertEnter,InsertLeave', '<buffer>', function() refresh_syntax(true) end)
  autocmd('TextChanged', '<buffer>', function() refresh_syntax(true) end)
